import { decode } from '@automerge/automerge-repo/helpers/cbor.js';
...
import { MessageContents, Synchronizer } from './synchronizer.js';
...
export class DocSynchronizer extends Synchronizer {
	...
	beginSync(peerIds: PeerId[]) {
		...
		const doc = this.#handle.docSync() ?? A.init<unknown>();
	...
	#processSyncMessage(message: SyncMessage | RequestMessage) {
		...
		this.#withSyncState(message.senderId, syncState => {
			...
			this.#sendSyncMessage(message.senderId, this.#handle.docSync);
			return newDoc;
		});
	}
