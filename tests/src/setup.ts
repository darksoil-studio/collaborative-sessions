...
import { RealTimeSessionsClient } from '../../ui/src/real-time-sessions-client.js';
import { RealTimeSessionsStore } from '../../ui/src/real-time-sessions-store.js';

const testHappUrl = dirname(fileURLToPath(import.meta.url)) + '/../../workdir/real-time-sessions_test.happ';
...
async function addPlayer(scenario: Scenario) {
  const player = await scenario.addPlayerWithApp({ type: 'path', value: testHappUrl });

  patchCallZome(player.appWs as AppWebsocket);
  await player.conductor
  	.adminWs()
  	.authorizeSigningCredentials(player.cells[0].cell_id);
  const store = new RealTimeSessionsStore(
  	new RealTimeSessionsClient(player.appWs as any, 'real_time_sessions_test'),
  );
  return {
  	store,
  	player,
