...
export class ExampleClient extends ZomeClient<ExampleSignal> {
  ...
  async getLatestExample(exampleHash: ActionHash): Promise<EntryRecord<Example> | undefined> {
    const record: Record = await this.callZome("get_latest_example", exampleHash);
    return record ? new EntryRecord(record) : undefined;
  }

  async getOriginalExample(exampleHash: ActionHash): Promise<EntryRecord<Example> | undefined> {
    const record: Record = await this.callZome("get_original_example", exampleHash);
    return record ? new EntryRecord(record) : undefined;
  }

  async getAllRevisionsForExample(exampleHash: ActionHash): Promise<Array<EntryRecord<Example>>> {
    const records: Record[] = await this.callZome("get_all_revisions_for_example", exampleHash);
  ...
  getAllDeletesForExample(originalExampleHash: ActionHash): Promise<Array<SignedActionHashed<Delete>> | undefined> {
    return this.callZome("get_all_deletes_for_example", originalExampleHash);
  }

  getOldestDeleteForExample(originalExampleHash: ActionHash): Promise<SignedActionHashed<Delete> | undefined> {
